 ...
 * @param {import('xink').Node[]} [static_children] 
 * @returns {Node}
 */
...
/**
 * 
 * @param {import('xink').Node} node 
 * @param {string} new_segment 
 * @returns {import('xink').Node}
 */
 ...
 * @param {import('xink').Matcher} matcher 
 * @param {import('xink').MatcherType} matcher_type 
 * @returns {import('xink').ParametricNode}
 */
...
/**
 * 
 * @param {Map<string, import('xink').ParametricNode} data 
 * @returns {Map<string, import('xink').ParametricNode}
 */
const sortMap = (data) => {
 ...
 */
export class Router {
  /** @type {import('xink').Node} */
  _root
  /** @type {import('xink').StoreFactory} */
  _storeFactory
  /** @type {Map<string, import('xink').Matcher>} */
  _matchers
   ...
   * @param {import('xink').StoreFactory} [RouterArgs.storeFactory]
   */
   ...
   * 
   * @param {string} type 
   * @returns {import('xink').Matcher} Matcher
   */
  getMatcher(type) {
  ...
  register(path) {
    ...
    for (let i = 0; i < static_segments.length; ++i) {
      ...
      if (i > 0) { // Set parametric properties on the node
        ...
        let param_name = ''
        /** @type {import('xink').Matcher} */
        let matcher = null
        /** @type {import('xink').MatcherType} */
 ...
 * 
 * @param {string} url 
 * @param {number} url_length 
 * @param {import('xink').Node} node 
 * @param {number} start_index 
 * @returns {import('xink').Route}
 */
function matchRoute(url, url_length, node, start_index) {
  const { segment } = node;
  ...
  if (segment_len > 1) {
    ...
    if (segment_end_index > url_length) {
      //console.log('segment is longer than url. no match. return')
      return null;
    }
    ...
    if (segment_len < 15) { // Using a loop is faster for short strings
      ...
      for (let i = 1, j = start_index + 1; i < segment_len; ++i, ++j) {
        //console.log('comparing', segment[i], url[j])
        if (segment[i] !== url[j]) {
  ...
  if (start_index === url_length) { // Reached the end of the URL
    //console.log('reached end of url')
    if (node.store !== null) {
